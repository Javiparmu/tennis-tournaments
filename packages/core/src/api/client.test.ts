...
// client.ts reads its base URL + timeout from the injected config (setApiConfig),
// so env-dependent behavior is exercised by importing a fresh module and
// configuring it per test.
async function loadClient(cfg: { baseUrl?: string; timeoutMs?: number } = {}) {
  vi.resetModules();
  const clientMod = await import("./client");
  clientMod.setApiConfig({ baseUrl: cfg.baseUrl ?? "http://api.test", timeoutMs: cfg.timeoutMs ?? 8000 });
  return clientMod;
}
...
describe("configuration", () => {
  it("falls back to an 8000ms timeout when the configured timeout is not a valid number", async () => {
    const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout");
    const { request } = await loadClient({ timeoutMs: Number("abc") });
