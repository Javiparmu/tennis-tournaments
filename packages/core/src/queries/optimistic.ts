...
// The danger-toast channel for fire-and-close mutation failures is framework-
// specific (HeroUI on web, a native toast on mobile), so the host app injects it
// via setMutationNotifier() at startup. Defaults to a no-op until configured; the
// QueryClient's own mutations.onError floor still logs every failure.
type MutationErrorNotifier = (error: unknown) => void;

let notifyError: MutationErrorNotifier = () => {};

export function setMutationNotifier(notifier: MutationErrorNotifier): void {
