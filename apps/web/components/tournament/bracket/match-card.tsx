...
export function MatchCard({
  ...
  const disabled = selectionState?.disabled ?? false;
  ...
  const base = `block min-w-0 w-full overflow-hidden rounded-xl border bg-white text-left shadow-sm ${
    ...
    return (
      <button
        ...
        onClick={(event) => {
          ...
          if (start && event.detail !== 0 && Math.hypot(event.clientX - start.x, event.clientY - start.y) > 6) return;
          onSelect(match);
        }}
        disabled={disabled}
        ...
        className={`${base} ${
          disabled
            ? "opacity-60"
            : "cursor-pointer transition-shadow hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-court"
        }`}
      >
        {inner}
      </button>
