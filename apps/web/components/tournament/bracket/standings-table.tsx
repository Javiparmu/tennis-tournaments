import type { PlayerStanding } from "@courtrank/core/lib/standings";
...
export function StandingsTable({ rows }: { rows: PlayerStanding[] }) {
  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-court/15 px-3 py-4 text-center text-xs text-stone-400">
        ...
        <tbody className="divide-y divide-court/5">
          {rows.map(({ player, wins, losses }, i) => (
            <tr key={player.id} className={i === 0 ? "bg-ball/10" : undefined}>
              ...
              <td className="px-1 py-1.5 text-center font-mono text-stone-500">{wins + losses}</td>
              <td className="px-1 py-1.5 text-center font-mono font-semibold text-court">{wins}</td>
              <td className="px-1 py-1.5 text-center font-mono text-stone-500">{losses}</td>
            </tr>
          ))}
        </tbody>
      </table>
